...
class Person : IComparable 
{
    ...
    //Метод, который находит наибольший элемент в массиве Person[]
    public static Person maxPerson(Person[] persons)
    {
        Person personmax = persons[0];
        foreach (Person person in persons)
        {
            if (person.CompareTo(personmax) == 1)
                personmax = person;
        }
        return personmax;
    }
    ...
    //Метод, который находит наибольший элемент в массиве Person[]
    public static Person maxPerson(Person[] persons, IComparer<Person> comparer)
    {
        Person personmax = persons[0];
        foreach (Person person in persons)
        {
            if (comparer.Compare(person, personmax) > 0)
                personmax = person;
        }
        return personmax;
    }
